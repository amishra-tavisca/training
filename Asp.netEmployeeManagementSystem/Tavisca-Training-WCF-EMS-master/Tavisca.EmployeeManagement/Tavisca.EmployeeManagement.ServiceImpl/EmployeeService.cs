...
namespace Tavisca.EmployeeManagement.ServiceImpl
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public DataContract.Employee GetByEmail(string employeeEmail)         //remove
        {
            try
            {
                var result = _manager.GetByEmail(employeeEmail);
                if (result == null) return null;
                return result.ToDataContract();
            }
            catch (Exception ex)
        ...
        public bool IsValid(string employeeEmail, string password)
        {
            try
            {
                var result = _manager.IsValid(employeeEmail,password);
                return result;
            }
            catch (Exception ex)
            {
